...
class Evaluator():
    ...
    def __init__(self, prompt_id, use_mask, out_dir, modelname, train_x, dev_x, test_x, train_y, dev_y, test_y):
    ...
    def evaluate(self, model,epoch,logger, print_info=False):
        #print(K.int_shape(self.train_x))
        if self.use_mask and 0:
            train_pred = model.predict([self.train_x, self.train_mask], batch_size=10).squeeze()
            dev_pred = model.predict([self.dev_x, self.dev_mask], batch_size=10).squeeze()
            test_pred = model.predict([self.test_x, self.test_mask], batch_size=10).squeeze()
        else:
            train_pred = model.predict(self.train_x, batch_size=10).squeeze()
            dev_pred = model.predict(self.dev_x, batch_size=10).squeeze()
            test_pred = model.predict(self.test_x, batch_size=10).squeeze()
