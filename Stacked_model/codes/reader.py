...
def get_model_friendly_scores(scores_array, prompt_id_array):
    ...
    assert arg_type in {int, np.ndarray, list}
    ...
    if arg_type is list:
        for i in range(len(scores_array)):
            ...
            scores_array[i] =  float((scores_array[i]- low) / (high - low))
    return scores_array
...
def create_vocab(file_path, prompt_id, vocab_size, tokenize_text, to_lower):
    print('Creating vocabulary from: ' + file_path)
...
def get_data(paths, vocab_path, prompt_id, vocab_size, tokenize_text=True, to_lower=True, sort_by_len=False):
    ...
    logger.info("Data max sentence num = %s, max sentence length = %s" % (train_maxsentnum, train_maxsentlen))
    ...
    return (train_x, train_y, train_prompts), vocab, train_maxsentlen, train_maxsentnum
